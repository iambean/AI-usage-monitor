...
应用初始化时只启用 Codex，并将其设为默认数据源。你可以在设置中启用其他数据源；默认数据源有且仅有一个，由菜单栏常驻显示所选窗口的摘要。设置中支持调整数据源顺序。
...
- 菜单栏仅显示图标和用量值，点击展开后查看详细数据
- 默认数据源有且仅有一个；支持选择摘要窗口和调整数据源顺序
- 每个窗口独立显示额度、进度条和重置倒计时，悬停可查看精确时间与时区
- Codex 显示可用重置次数和有效期；手动重置需要确认，结果不明时复用同一次请求
- 主面板和设置窗口遵循 macOS 自然窗口层级，切换应用后不强制置顶
- 设置内可切换跟随系统、简体中文或 English，默认跟随系统并以简体中文作为回退语言
- 可获取账号资料的数据源显示完整账号，长名称自动换行
- 自动发现本机安装的 Codex、Claude Code 和 Kimi Code
- API 型数据源的凭证保存在 macOS 钥匙串
- 默认数据源保持正常刷新，次要数据源每小时后台刷新，展开面板时立即刷新全部
- 可配置低电量模式刷新与唤醒检查，支持逐服务刷新和独立更新时间
- 已知的短暂网络故障保留并标明上次数据；凭证失效或无法识别的数据清空旧额度
- 趋势沿用扁平曲线绘制与缓存查询，默认显示全部窗口及本机历史，兼容升级前未标记账户的旧记录
- 支持按窗口、记录来源和单位筛选；采样断档留白，记录重置和额度变化事件
- 连续样本充足时提供明确标注的耗尽时间估算
- 支持 7 / 30 / 90 天历史保留期限（最多 10,000 条采样）、CSV 导出和确认后清除
- 通知默认关闭；开启后可按窗口设置低额度、低余额、恢复和重置到期提醒，支持静默时段与去重
- 支持紧凑布局及跟随系统、浅色、深色外观
- 支持导出本地脱敏诊断包，不接入第三方遥测
- 每天最多在后台检查一次 GitHub Releases 新版本
- 通过 Sparkle 在应用内完成更新下载、签名验证、安装和重新启动
- 支持登录时启动
...
| 数据源 | 状态 | 配置方式 | 展示内容 |
| --- | --- | --- | --- |
| Codex | 可用，默认启用 | 自动发现本机 Codex CLI | 剩余百分比、各用量周期、自动重置时间、可用手动重置次数和有效期 |
...
| MiniMax | 可用，尽力兼容 | Token Plan 订阅 Key / sk-api- 按量计费 Key | 订阅额度与重置周期 / API 可用余额（中国站人民币、国际站美元；接口返回币种时优先采用） |
...
- 可配置低电量模式刷新与唤醒检查，支持逐服务刷新和独立更新时间，但始终保留手动刷新。
...
- 通过版本标签自动构建 Apple Silicon 安装包、签署 Sparkle 更新包、更新 appcast 并发布到 GitHub Releases，支持应用内安装。
